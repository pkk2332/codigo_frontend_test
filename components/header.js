...
const Header = () => {
	...
	const openNav = () => {
		...
		if (isNavOpen) {
			...
			setTimeout(() => navitem_animation(), 500);
		}
		return setstate((prev) => setstate(!prev));
	};
	...
	return (
		<header className="nav-bar relative op z-10">
			...
			<div className={' nav-items ' + (isNavOpen ? 'nav__expended' : '')}>
				...
				<a className={(!isNavOpen ? 'hidden ' : '') + 'nav-item lg:hidden '} href="#">
					Request a quote
				</a>
				<a className={(!isNavOpen ? 'hidden ' : '') + 'nav-item lg:hidden '} href="#">
					Lets Chat
				</a>
				<a onClick={openNav} className="cross_icon z-50">
					<svg
						style={{ fill: 'white' }}
						height="20"
